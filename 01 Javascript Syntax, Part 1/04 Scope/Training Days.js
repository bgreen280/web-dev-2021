...
// 9 Try the functions for another competitor. Copy and paste this code at the end of the file. const event2 = getRandEvent(); const days2 = getTrainingDays(event2); const name2 = 'Warren'; logEvent(name2, event2); logTime(name2, days2);
...
////////////////////////////////////////
// moved random inside get rand event block to generate new event each instance
const getRandEvent = () => {
  const random = Math.floor(Math.random() * 3);
  if (random === 0) {
    return 'Marathon';
...
// created 'days' variable to be manipulated within block scope
const getTrainingDays = event => {
  let days;  
  if (event === 'Marathon') {
    days = 50;
...
const name = 'Nala'; // created a globally scoped name 
const event = getRandEvent(); 
const days = getTrainingDays(event);
...
// test program with second participant 
