...
// 12 For extra practice, try these: getActualSleepHours() could be implemented without calling getSleepHours(). Use literal numbers and the + operator to rewrite getActualSleepHours(). It should still return the total actual hours slept in the week. Some people need to sleep longer than others. Rewrite getIdealSleepHours() so that you can pass it an argument, like getIdealSleepHours(8) where 8 is the ideal hours per night. Update the call to getIdealSleepHours() in calculateSleepDebt() too. To see the solutions, open the hint.

// Solution:
// create function to output hours slept a given night of the week
...
// test sleep hours function
// getSleepHours('monday')

// create actual and ideal sleep hours
const getActualSleepHours = () => 8 + 8 + 8 + 8 + 8 + 8 + 8
const getIdealSleepHours = idealHours => idealHours * 7;

// test actual and ideal sleep hours
// console.log(getActualSleepHours());
// console.log(getIdealSleepHours());

// calculate sleep debt, output hours slept and 'amount of sleep'
const calculateSleepDebt = () => {
  const actualSleepHours = getActualSleepHours();
  const idealSleepHours = getIdealSleepHours(8);

  if (actualSleepHours === idealSleepHours) {
    console.log(`You have slept ${actualSleepHours} hours this week.  That's the perfect amount of sleep!`)
  } else if (actualSleepHours > idealSleepHours) {
    console.log(`You have slept ${actualSleepHours} hours this week.  That's more than enough sleep!`)
  } else if (actualSleepHours < idealSleepHours) {
    console.log(`You have slept ${actualSleepHours} hours this week.  That's not enough sleep!`)
  };
};

// call calculate sleep debt function for testing
calculateSleepDebt()